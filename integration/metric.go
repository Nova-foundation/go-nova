...
func (db *DBProducerWithMetrics) OpenDB(name string) (kvdb.Store, error) {
	...
	dm.diskSizeGauge = metrics.GetOrRegisterGauge("nova/chaindata/"+name+"/disk/size", nil)
	if strings.HasPrefix(name, "gossip-") || strings.HasPrefix(name, "lachesis-") {
		name = "epochs"
	}
	logger := log.New("database", name)
	dm.log = logger
	dm.diskReadMeter = metrics.GetOrRegisterMeter("nova/chaindata/"+name+"/disk/read", nil)
	dm.diskWriteMeter = metrics.GetOrRegisterMeter("nova/chaindata/"+name+"/disk/write", nil)
