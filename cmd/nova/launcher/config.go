...
import (
	...
	"github.com/Nova-foundation/go-nova/integration"
	...
	futils "github.com/Nova-foundation/go-nova/utils"
...
var (
	...
	RPCGlobalGasCapFlag = cli.Uint64Flag{
		Name:  "rpc.gascap",
		Usage: "Sets a cap on gas that can be used in nvt_call/estimateGas (0=infinite)",
		Value: gossip.DefaultConfig(cachescale.Identity).RPCGasCap,
	}
	RPCGlobalTxFeeCapFlag = cli.Float64Flag{
		Name:  "rpc.txfeecap",
		Usage: "Sets a cap on transaction fee (in NVT) that can be sent via the RPC APIs (0 = no cap)",
...
func mayGetGenesisStore(ctx *cli.Context) *genesisstore.Store {
	...
	case ctx.GlobalIsSet(FakeNetFlag.Name):
		...
		return makefakegenesis.FakeGenesisStore(num, futils.ToNvt(1000000000), futils.ToNvt(5000000))
	case ctx.GlobalIsSet(GenesisFlag.Name):
		...
		// check if it's a trusted preset
		{
			...
			for _, allowed := range AllowedNovaGenesis {
...
func mayMakeAllConfigs(ctx *cli.Context) (*config, error) {
	...
	cfg := config{
		...
		Nova:          gossip.DefaultConfig(cacheRatio),
		Emitter:       emitter.DefaultConfig(),
		TxPool:        evmcore.DefaultTxPoolConfig,
		NovaStore:     gossip.DefaultStoreConfig(cacheRatio),
	...
	var err error
	cfg.Nova, err = gossipConfigWithFlags(ctx, cfg.Nova)
	if err != nil {
		return nil, err
	}
	cfg.NovaStore, err = gossipStoreConfigWithFlags(ctx, cfg.NovaStore)
	if err != nil {
		return nil, err
	}
	...
	if err := cfg.Nova.Validate(); err != nil {
		return nil, err
	}
...
func defaultNodeConfig() node.Config {
	...
	cfg.HTTPModules = append(cfg.HTTPModules, "eth", "nvt", "dag", "abft", "web3")
	cfg.WSModules = append(cfg.WSModules, "eth", "nvt", "dag", "abft", "web3")
	cfg.IPCPath = "nova.ipc"
