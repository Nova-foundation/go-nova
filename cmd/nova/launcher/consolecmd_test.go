...
const (
	ipcAPIs  = "abft:1.0 admin:1.0 dag:1.0 debug:1.0 nvt:1.0 net:1.0 personal:1.0 rpc:1.0 txpool:1.0 web3:1.0"
	httpAPIs = "abft:1.0 dag:1.0 nvt:1.0 rpc:1.0 web3:1.0"
)

// Tests that a node embedded within a console can be started up properly and
// then terminated by closing the input stream.
func TestConsoleWelcome(t *testing.T) {
	// Start an nova console, make sure it's cleaned up and terminate the console
	cli := exec(t,
...
instance: go-nova/v{{version}}/{{goos}}-{{goarch}}/{{gover}}
coinbase: {{.Coinbase}}
at block: 1 ({{niltime}})
 datadir: {{.Datadir}}
 modules: {{apis}}
...
func testAttachWelcome(t *testing.T, cli *testcli, endpoint, apis string) {
	// Attach to a running nova node and terminate immediately
