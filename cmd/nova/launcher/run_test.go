...
import (
	...
	"github.com/Nova-foundation/go-nova/cmd/cmdtest"
)

func tmpdir(t *testing.T) string {
	dir, err := ioutil.TempDir("", "nova-test")
	if err != nil {
		t.Fatal(err)
	}
	return dir
...
func init() {
	// Run the app if we've been exec'd as "nova-test" in exec().
	reexec.Register("nova-test", func() {
...
func exec(t *testing.T, args ...string) *testcli {
	...
	// Boot "nova". This actually runs the test binary but the TestMain
	// function will prevent any tests from running.
	tt.Run("nova-test", args...)
