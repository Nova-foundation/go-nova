...
import (
	...
	"github.com/Nova-foundation/go-nova/cmd/nova/launcher/metrics"
	"github.com/Nova-foundation/go-nova/cmd/nova/launcher/tracing"
	"github.com/Nova-foundation/go-nova/debug"
	"github.com/Nova-foundation/go-nova/evmcore"
	"github.com/Nova-foundation/go-nova/flags"
	...
	"github.com/Nova-foundation/go-nova/utils/errlock"
	"github.com/Nova-foundation/go-nova/valkeystore"
	_ "github.com/Nova-foundation/go-nova/version"
)

const (
	// clientIdentifier to advertise over the network.
	clientIdentifier = "go-nova"
)
...
var (
	...
	app = flags.NewApp(gitCommit, gitDate, "the go-nova command line interface")
...
func initFlags() {
	...
	nodeFlags = append(nodeFlags, novaFlags...)
...
// nova is the main entry point into the system if no special subcommand is ran.
...
func makeNode(ctx *cli.Context, cfg *config, genesisStore *genesisstore.Store) (*node.Node, *gossip.Service, func()) {
	...
	svc, err := gossip.NewService(stack, cfg.Nova, gdb, blockProc, engine, dagIndex, newTxPool, haltCheck)
	if err != nil {
...
func startNode(ctx *cli.Context, stack *node.Node) {
	...
	// Create a client to interact with local nova node.
